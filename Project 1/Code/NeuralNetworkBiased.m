...
NNInitializeWeights = {@InitializeWeightsBasic, @InitializeWeightsBiased};
NNClassificationLoss = ...
    {@(weightLinear, index) ClassificationLossBasic(weightLinear, ...
    Xtrain(index,:), yExpanded(index,:), nHidden, nLabel), ...
    @(weightLinear, index) ClassificationLossBiased(weightLinear, ...
    Xtrain(index,:), yExpanded(index,:), nHidden, nLabel)};
NNClassificationPredict = ...
    {@ClassificationPredictBasic, ...
    @ClassificationPredictBiased};

for indexTrial = 1:2
    weightLinear = NNInitializeWeights{indexTrial}(dim, nHidden, nLabel);
    for iter = 1:iterMax
        ...
        [~, gradLinear] = ...
            NNClassificationLoss{indexTrial}(weightLinear, indexTrain);
...
legend('without bias', 'with bias')
xlabel('Iteration');
ylabel('Error rate');
title('Validation Set Error Rate with or without Bias in the Network');